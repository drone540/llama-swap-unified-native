...
- **Source repositories are cloned automatically** into the directory next to
  `install-ai-stack.sh` (e.g. `llama.cpp/`, `whisper.cpp/`) when they don't
  already exist. If a directory already exists it is left untouched — you can
  `git pull` to update or `git checkout` a specific revision to build it.
...
- Components are compiled locally, so builds take time; only the components you
  select are built.
- `--uninstall` removes the installed software but leaves the source clones and
  downloaded models unless you pass `--purge-data`.
